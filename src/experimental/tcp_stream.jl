...
function Base.push!(stream::TCPStream, iheader::IPv4Header, packet::TCPPacket)
    ...
    elseif theader.FIN
        ...
        elseif theader.seq_num < src_endpoint.seq
            ...
            @warn "TCP retransmission" src_endpoint dst_endpoint iheader packet
            stream.event_handler(src_endpoint.addr, dst_endpoint.addr, TCPE_RETRANSMIT)
            # NOTE: this is not invalid, but unusual situation, not currently supported
            if (theader.seq_num + length(packet.payload)) % UInt32 > src_endpoint.seq
                @error "TCP retransmission beyond current seq_num" src_endpoint dst_endpoint iheader packet
                stream.event_handler(src_endpoint.addr, dst_endpoint.addr, TCPE_ERROR)
            end
            return
        end
